...
OpGenerator vectorCompressStoreGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    if (maskCandidates.empty()) {
      auto vectorTy =
          VectorType::get(mlir::dyn_cast<ShapedType>(vectorOperand.type).getShape(),
                          builder.getI1Type());
...
OpGenerator vectorContractGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    std::vector<vector::CombiningKind> kinds =
        mlir::dyn_cast<IntegerType>(lhsVTy.getElementType()) ? intKinds : floatKinds;
...
OpGenerator vectorExpandLoadGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    auto maskTy = VectorType::get(mlir::dyn_cast<VectorType>(resultTy).getShape(),
                                  builder.getI1Type());
...
OpGenerator vectorGatherGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    auto baseTy = mlir::dyn_cast<ShapedType>(randomMemrefOrRankedTensorType(builder.getContext()));
...
OpGenerator vectorLoadGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    auto memrefCandidates = region.pool.searchCandidatesFrom(
        {PoolType::StaticShapedMemref, PoolType::DynamicShapedMemref},[](TypeValue tval) {
          return mlir::dyn_cast<ShapedType>(tval.type).getRank() == 1; // in the -convert-vector-to-llvm pass defining file, only vector with one rank can be lowered
        });
    if (memrefCandidates.empty()) {
      std::vector<int64_t> shape; // in the -convert-vector-to-llvm pass defining file, only vector with one rank can be lowered
      if (UR(2)) {
        shape.push_back(dimPool[UR(dimPool.size())]);
      } else {
    ...
    // else
    //   shape.push_back(ShapedType::kDynamic);
    auto vectorTy = random1DVectorType(memrefTy.getElementType()); // in the -convert-vector-to-llvm pass defining file, only vector with one rank can be lowered
...
OpGenerator vectorMaskGenerator() {
  ...
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    if (true) {
      ...
      std::vector<vector::CombiningKind> kinds =
          mlir::dyn_cast<IntegerType>(vTy.getElementType()) ? intKinds : floatKinds;
      auto kind = kinds[UR(kinds.size())];
    ...
    auto maskTy = VectorType::get(
        mlir::dyn_cast<ShapedType>(operand.type).getShape(), builder.getI1Type());
    auto maskCandidates = region.pool.searchCandidatesFrom(
        {PoolType::Vector},
        [&](TypeValue tval) { return tval.type == maskTy; });
    if (maskCandidates.empty()) {
...
OpGenerator vectorReductionGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    std::vector<vector::CombiningKind> kinds =
        mlir::dyn_cast<IntegerType>(mlir::dyn_cast<VectorType>(src.type).getElementType())
            ? intKinds
            : floatKinds;
    auto kind = kinds[UR(kinds.size())];
...
OpGenerator vectorScatterGenerator() {
  ...
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    auto vecToStoreTy = VectorType::get(
        indexVTy.getShape(), mlir::dyn_cast<MemRefType>(base.type).getElementType());
...
OpGenerator vectorShuffleGenerator() {
  return [](OpBuilder &builder, Location loc, OpRegion &region) {
    ...
    auto operand1Candidates =
        pool.searchCandidatesFrom({PoolType::Vector}, [&](TypeValue tVal) {
          return hasSameTrailingShape(mlir::dyn_cast<VectorType>(tVal.type), vTy0);
        });
