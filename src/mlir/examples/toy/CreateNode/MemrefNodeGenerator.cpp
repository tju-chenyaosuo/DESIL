...
OpGenerator memrefStoreGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    builder.create<memref::StoreOp>(loc, tValToStore, alloc, storeIndices);
    return true;
  };
}
...
OpGenerator tensorStoreGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    builder.create<bufferization::MaterializeInDestinationOp>(loc, tensor, destMem);
    return true;
  };
}
...
OpGenerator genericAtomicRMWGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    auto op = builder.create<memref::GenericAtomicRMWOp>(loc, memref,
                                                         ValueRange(indices));
