...
OpGenerator vectorBroadcastGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value vector =
        builder.create<vector::BroadcastOp>(loc, vectorTy, operand);
    mb->add2Pool(vector);
    return true;
  };
}
...
OpGenerator vectorBitCastGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value vector =
        builder.create<vector::BitCastOp>(loc, resultTy, operand);
    mb->add2Pool(vector);
    return true;
  };
}
...
OpGenerator vectorCompressStoreGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    builder.create<vector::CompressStoreOp>(loc, memref, indices, maskOperand,
                                            vectorOperand);
    return true;
  };
}
...
OpGenerator vectorConstantMaskGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value val =
        builder.create<vector::ConstantMaskOp>(loc, vectorTy, arrAttr);
    mb->add2Pool(val);
    return true;
  };
}
...
OpGenerator vectorCreateMaskGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value val =
        builder.create<vector::CreateMaskOp>(loc, vectorTy, indices);
    mb->add2Pool(val);
    return true;
  };
}
...
OpGenerator vectorContractGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    builder.create<vector::ContractionOp>(loc, vectorLhs, vectorRhs, accOperand,
...
OpGenerator vectorExtractGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value res =
        builder.create<vector::ExtractOp>(loc, vectorOperand, arrAttr);
    mb->add2Pool(res);
    return true;
  };
}
...
OpGenerator vectorFMAGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value res =
        builder.create<vector::FMAOp>(loc, lhsOperand, rhsOperand, accOperand);
    mb->add2Pool(res);
    return true;
  };
}
...
OpGenerator vectorInsertStridedSliceGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    builder.create<vector::InsertStridedSliceOp>(loc, srcVector, destVector,
                                                 offsets, strides);
    return true;
  };
}
...
OpGenerator vectorLoadGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value res =
        builder.create<vector::LoadOp>(loc, vectorTy, memref, indices);
    mb->add2Pool(res);
    return true;
  };
}
...
//     mlir::Value res = builder.create<vector::OuterProductOp>(loc, acc,
//     lhs,rhs, acc, kind);
...
OpGenerator vectorScanGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    auto results = builder.create<vector::ScanOp>(loc, kind, src, init,
...
OpGenerator vectorScatterGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    builder.create<vector::ScatterOp>(loc, base, ValueRange(indices), indexVec,
                                      mask, vecToStore);
    return true;
  };
}
...
OpGenerator vectorShuffleGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    mlir::Value res =
        builder.create<vector::ShuffleOp>(loc, operand0, operand1, mask);
    mb->add2Pool(res);
    return true;
  };
}
...
OpGenerator vectorTransferReadGenerator() {
  return [](OpBuilder &builder, Location loc, MutationBlock *mb) -> bool {
    ...
    if (rollIdx(2)) {
      builder.create<vector::TransferReadOp>(loc, vecTy, src, indices,
                                             permutationMapAttr, nullptr);
    } else {
      builder.create<vector::TransferReadOp>(loc, vecTy, src, indices, padding,
